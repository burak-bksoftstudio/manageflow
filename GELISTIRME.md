...
| Mevcut sürüm | `0.25.0-production-foundation` |
| Mevcut aşama | Vercel production yayını, GitHub bağlantısı ve güvenli Auth callback temeli hazır |
| Sonraki ana hedef | Supabase production Auth adreslerini tanımlayıp gerçek şifre yenileme akışını doğrulamak |
...
- Uygulama Vercel production ortamında `https://manageflow-seven.vercel.app` adresinde yayınlanmaktadır; doğrudan SPA rotaları ve temel güvenlik başlıkları aktiftir.
...
| Production yayını | Vercel projesi ve GitHub repository bağlantısı hazır; production Auth URL ayarı bekliyor |
...
| Kimlik doğrulama | Kısmen hazır | Kayıt, doğrulama, giriş, çıkış ve kalıcı oturum doğrulandı; recovery callback koruması hazır, şifre yenileme teslim testi bekliyor |
...
| Deployment | Kısmen hazır | Vercel production yayını, GitHub bağlantısı, SPA rewrite ve ortam değişkenleri hazır; özel domain ve production Auth URL testi bekliyor |
...
Auth ekranları masaüstü ve mobil yerleşimde görsel olarak doğrulandı. Yerel URL izin listesi yapılandırıldı; gerçek hesapla kayıt, e-posta doğrulama, çıkış ve yeniden giriş tamamlandı. Production URL izin listesi Supabase panelinde tanımlandıktan sonra şifre yenileme e-posta teslimi ayrıca test edilmelidir.
...
- Canlı uygulama adresi: `https://manageflow-seven.vercel.app`
...
Vercel production yayını çalışmaktadır. Supabase Dashboard içindeki `Site URL` ve kesin production redirect adresleri kullanıcı tarafından tanımlanmadan canlı e-posta doğrulama, şifre yenileme ve davet callback'leri tamamlanmış kabul edilmez.
...
- [ ] Supabase Auth production Site URL ve redirect adresleri
- [ ] Sentry hata takibi
- [ ] Ürün analitiği
...
1. Supabase Auth `Site URL` değerini `https://manageflow-seven.vercel.app` olarak yapılandır.
2. Yerel ve production doğrulama, şifre yenileme ve davet kabul adreslerini kesin redirect listesine ekle.
3. Gerçek şifre sıfırlama e-postası teslimini ve `/sifre-yenile` dönüş akışını uçtan uca doğrula.
4. Canlı ortamda kayıt, giriş, davet, müşteri → proje → görev ve ayarlar smoke testini tamamla.
5. İlk beta öncesinde özel domain seçimini ve SMTP sağlayıcısını kesinleştir.
...
- Supabase Auth production `Site URL` ve kesin redirect adresleri Dashboard'da kullanıcı tarafından henüz tanımlanmadı.
- Gerçek production şifre yenileme e-posta teslimi bu ayardan sonra test edilmelidir.
