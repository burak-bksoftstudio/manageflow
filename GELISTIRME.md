...
| Mevcut sürüm | `0.14.0-project-team` |
| Mevcut aşama | Proje yaşam döngüsü ve gerçek ekip üyesi atama/çıkarma akışları Supabase ile çalışıyor |
| Sonraki ana hedef | Projeye bağlı gerçek görev veri modeli, görev listesi ve ilk görev oluşturma akışı |
...
Proje CRUD yaşam döngüsü ve proje ekibi yönetimi tamamlanmıştır. Projeye bağlı görev temeli sıradaki pakettedir.
...
- Projeye bağlı kalıcı görev kayıtları ve görev atamaları
...
1. Aktif organizasyona ve zorunlu projeye bağlı temel `tasks` veri modelini oluştur.
2. Yapılacak, devam ediyor, incelemede ve tamamlandı görev durumlarını tanımla.
3. Düşük, normal, yüksek ve acil öncelik seviyelerini ekle.
4. Organizasyon üyelerine görev okuma; yetkili rollere oluşturma/güncelleme RLS kurallarını yaz.
5. Mevcut `/gorevler` placeholder'ını gerçek responsive görev listesiyle değiştir.
6. İlk görev oluşturma modalında proje ve isteğe bağlı proje üyesi seçimini gerçek veriden getir.

Sıradaki ManageFlow geliştirme paketinin başarı ölçütü:

```text
Yetkili kullanıcı aktif projeyi seçerek görev oluşturabilir
→ Göreve başlık, durum, öncelik ve bitiş tarihi verebilir
→ İsterse projeye atanmış bir ekip üyesini görevlendirebilir
→ Sayfa yenilendiğinde görev korunur
→ Organizasyon üyeleri görevleri okuyabilir
→ Başka organizasyonun proje veya üyesi göreve bağlanamaz
```
