...
| Mevcut aşama | Vercel production yayını ve temel Auth yaşam döngüsü canlı ortamda doğrulandı |
| Sonraki ana hedef | Proje bağlantılı güvenli dosya yükleme ve listeleme temelini oluşturmak |
...
| Production yayını | Vercel, GitHub deployment ve production Auth callback bağlantıları çalışıyor; özel domain bekliyor |
...
| Deployment | Kısmen hazır | Vercel production yayını, GitHub bağlantısı, SPA rewrite, ortam değişkenleri ve Auth callback'leri hazır; özel domain bekliyor |
...
Vercel production yayını çalışmaktadır. Supabase Dashboard içindeki `Site URL` ve kesin yerel/production redirect adresleri tanımlanmıştır. Gerçek şifre sıfırlama e-postasının canlı adrese dönüşü ve yeni şifre kaydı uçtan uca doğrulanmıştır.
...
- Production kayıt doğrulama ve davet kabul callback'lerinin canlı hesaplarla tekrar smoke testi
- Google ile giriş
...
1. Organizasyon ve proje bağlantılı dosya metadata modelini ve private Supabase Storage bucket'ını oluştur.
2. Dosya yükleme, listeleme ve güvenli indirme servislerini RLS/Storage politikalarıyla bağla.
3. `/dosyalar` sayfasını gerçek veriye geçir; proje filtresi, boş, yükleniyor ve hata durumlarını ekle.
4. Dosya boyutu/türü doğrulaması ile yükleyen kullanıcı ve organizasyon bütünlüğünü koru.
5. Yetkili silme akışını, domain testlerini ve uzak RLS/storage kontrollerini tamamla.

Sıradaki ManageFlow geliştirme paketinin başarı ölçütü:

```text
Yetkili ekip üyesi aktif organizasyonundaki bir projeye dosya yükleyebilir
→ Dosya private bucket içinde organizasyon/proje yolu altında saklanır
→ Yalnız ilgili organizasyon üyeleri dosyayı listeleyip güvenli bağlantıyla indirebilir
→ Yetkisiz kullanıcı başka organizasyonun dosya kaydına veya nesnesine erişemez
→ Yenileme sonrasında dosya kaydı ve proje bağlantısı korunur
```
