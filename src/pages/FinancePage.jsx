...
import {
  ...
  getFinanceSummary, getInitialFinanceData, getPartnerBalances,
  ...
  return (
    <div className="finance-page" style={{ '--finance-color': config.color }}>
      <section className="finance-hero">
        <div><div className="eyebrow"><i /> {config.title.toUpperCase()} · FİNANS</div><h1>Paranın net resmi.</h1><p>Tahsilat, gider, ortak payı ve ödeme günleri — hepsi tek yerde.</p></div>
        <div className="finance-hero-actions"><button className="soft-button" onClick={lock}><LockKeyhole /> Kilitle</button><button className="agenda-button" onClick={() => setModal('income')}><Plus /> Gelir ekle</button><button className="agenda-button finance-expense-button" onClick={() => setModal('expense')}><Plus /> Gider ekle</button></div>
      </section>
      ...
      <nav className="finance-tabs"><button className={view === 'overview' ? 'active' : ''} onClick={() => setView('overview')}><WalletCards /> Genel bakış</button><button className={view === 'cash' ? 'active' : ''} onClick={() => setView('cash')}><Landmark /> Kasa & Banka</button><button className={view === 'clients' ? 'active' : ''} onClick={() => setView('clients')}><Building2 /> Firmalar</button><button className={view === 'partners' ? 'active' : ''} onClick={() => setView('partners')}><Users /> Ortaklar</button><button className={view === 'reports' ? 'active' : ''} onClick={() => setView('reports')}><ReceiptText /> Raporlar</button><button className={view === 'calendar' ? 'active' : ''} onClick={() => setView('calendar')}><CalendarDays /> Takvim</button></nav>
      {view === 'overview' ? <>
