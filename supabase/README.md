...
Test geçici ikinci organizasyon, davet, müşteri, proje, görev, checklist ve yorum kayıtları üretir; bütün işlemler tek transaction içinde çalışır ve en sonda `ROLLBACK` uygular. Başarılı sonuç `result: passed` değerini ve bütün güvenlik kontrollerini `true` olarak döndürür. Testin gerçek member hesabı bulabilmesi için bağlı projede en az bir aktif `member` üyeliği olmalıdır.
