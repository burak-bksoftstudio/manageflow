...
Owner/admin ekip timesheet raporunun rol sınırı, organizasyon kapsamı ve tarih aralığı ayrıca şu komutla doğrulanır:

```bash
supabase db query --linked --file supabase/tests/team_timesheet_rls_smoke.sql
```
...
Çalışma Alanı notlarının organizasyon izolasyonu, yazar/yönetici yetkisi, bağımsız bağlamı, etiket sınırı, sabitleme ve korumalı arşiv/geri yükleme yaşam döngüsü şu komutla doğrulanır:

```bash
supabase db query --linked --file supabase/tests/project_notes_rls_smoke.sql
```
...
İlk yirmi beş migration kimlik, organizasyon, davet, müşteri, proje, ekip, görev, zaman takibi ve çalışma alanı temelini kurar. Bu migration'lar `manageflow` uzak projesine uygulanmıştır.

`20260725220000_create_finance_system.sql`; organizasyon bazlı banka/kasa, cari, sözleşme, tahakkuk, kısmi tahsilat, ortak dağılımı, belge, bütçe, KDV/tevkifat ve değişiklik geçmişi temelini ekler. Finans tabloları yalnız owner/admin rollerine açıktır ve doğrudan silme yetkisi verilmez. Sözleşmelerden aylık tahakkuk üretmek için `finance_generate_contract_entries(ayın_ilk_günü)` RPC'si kullanılır.

Finans migration'ı bağlı `manageflow` projesine uygulanmıştır. Sonraki finans şema değişiklikleri yine ayrı migration olarak hazırlanmalı ve `supabase db push` öncesinde doğrulanmalıdır. CLI bağlantısının ürettiği `supabase/.temp/` klasörü makineye özeldir ve Git'e eklenmez.
